  ...
  try:
    while stream.isOpened():
      ...
      for idx in range(detected_face.shape[2]):
        ...
        # Main blurring loop.
        if face_blur:
          frame[face_tly:face_bry, face_tlx:face_brx] = cv2.blur(
              frame[face_tly:face_bry, face_tlx:face_brx], (25, 25))
      ...
      if cv2.waitKey(5) & 0xFF == ord('b'):
        face_blur = not face_blur
      cv2.imshow('MLE - Motion Detection', frame)
    else:
      print(f'Stream broke at {datetime.now()}')
      time.sleep(5.0)
  except cv2.error as error:
    disconnect(stream)
    raise error('Something went wrong with OpenCV.')
