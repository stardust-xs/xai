...
#
# ======================================================================
"""The `mle.core.weather.run` module.
...
background recording the weather of Mumbai every 45 mins and logging it
in a csv file on a daily basis.

The generated reports are stored under `./mle/data/weather/` directory
...
from mle.core.weather.track import weather
from mle.utils.common import check_internet, mle_path, now, toast
...
data_path = os.path.join(mle_path, 'data\\weather')
toast('MLE Weather Tracker', 'Weather tracking service started.')
  ...
  if (start_time >= next_weather_check_time
    ...
    if check_internet():
      try:
        ...
        toast('MLE Weather Tracker - Error Notification',
              'Internet connection is questionable.')
      except PermissionError:
        toast('MLE Weather Tracker - Error Notification',
              'File accessed by another application.')
    else:
      toast('MLE Weather Tracker - Skipping Track',
